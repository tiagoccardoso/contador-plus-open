...
class AlepProposicoesTab extends StatefulWidget {
  final String? deputado;

  const AlepProposicoesTab({super.key, required this.deputado});

  @override
  State<AlepProposicoesTab> createState() => _AlepProposicoesTabState();
}

class _AlepProposicoesTabState extends State<AlepProposicoesTab> {
  ...
  @override
  void didUpdateWidget(covariant AlepProposicoesTab oldWidget) {
  ...
  Future<Map<String, dynamic>> _load({bool forceRefresh = false}) async {
    ...
    return _api.proposicaoFiltrar(body, forceRefresh: forceRefresh);
  }
  ...
  void _publishShare(BuildContext context, String text) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      store.update(AlepTabKey.proposicoes, text);
    });
  }
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snap) {
        ...
        if (snap.hasError) {
          _publishShare(
            ...
            [
              ...
              'Deputado: ${widget.deputado}',
              '',
              'Falha ao carregar proposições pela API.',
              'Erro: ${snap.error}',
              '',
              'Consultas: https://consultas.assembleia.pr.leg.br/',
          ...
          return Center(
            child: Padding(
              ...
              child: Text('Falha ao carregar proposições.\n${snap.error}'),
            ),
          );
        }
        ...
        if (list.isEmpty) {
          _publishShare(
            ...
            [
              ...
              'Deputado: ${widget.deputado}',
              '',
              'Nenhuma proposição encontrada (no filtro atual).',
              '',
              'Consultas: https://consultas.assembleia.pr.leg.br/',
              'Fonte: ALEP',
            ].join('\n'),
          );
          return const Center(child: Text('Nenhuma proposição encontrada.'));
        }

        String oneLine(Map<String, dynamic> e) {
          final tipo = (e['tipoProposicao'] ?? '').toString().trim();
          final numero = (e['numero'] ?? '').toString().trim();
          final ano = (e['ano'] ?? '').toString().trim();
          final base = [tipo, if (numero.isNotEmpty && ano.isNotEmpty) '$numero/$ano' else numero]
        ...
        String link(Map<String, dynamic> e) {
          ...
          return 'https://consultas.assembleia.pr.leg.br/#/proposicao/$codigo';
        }
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, i) {
            ...
            final titulo = (e['ementa'] ?? e['assunto'] ?? e['tipoProposicao'] ?? 'Proposição').toString();
            final subt = [
              e['tipoProposicao']?.toString(),
              e['numero']?.toString(),
              e['ano']?.toString(),
              e['status']?.toString(),
            ...
            return ListTile(
              ...
              onTap: codigo == null
                  ? null
                  : () {
                      final uri = Uri.parse('https://consultas.assembleia.pr.leg.br/#/proposicao/$codigo');
                      openExternal(uri);
                    },
            );
          },
